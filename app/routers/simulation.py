    ...
    return [
        {
            ...
            "case_id": sim.case_id,
            "name": sim.name,
            "nurse_notes": sim.nurse_notes,
            "ergo_notes": sim.ergo_notes,
            "phys_notes": sim.phys_notes,
            "state": sim.state,
            "case": sim.case
        }
...
class SimulationCreate(BaseModel):
    ...
    ergo_notes: Optional[str] = None
    phys_notes: Optional[str] = None
...
async def create_simulation(
    ...
    case = db.query(CaseItem).filter(CaseItem.id == data.case_id).first()

    if not case:
        raise HTTPException(status_code=404, detail="Case doesn't exist, not creating simulation")
